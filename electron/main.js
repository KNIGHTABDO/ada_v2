...
function startPythonBackend() {
    ...
    // Use the local venv Python - works without Conda or any system Python setup
    const projectRoot = path.join(__dirname, '..');
    const venvPython = process.platform === 'win32'
        ? path.join(projectRoot, 'venv_ada', 'Scripts', 'python.exe')
        : path.join(projectRoot, 'venv_ada', 'bin', 'python');
