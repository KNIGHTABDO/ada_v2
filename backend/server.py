...
DEFAULT_SETTINGS = {
    ...
    "primary_model": "gpt-4o" # AI Model Selection
}
...
async def start_audio(sid, data=None):
    ...
    # Callback to notify frontend when TTS is speaking
    def on_tts_state(is_speaking):
        asyncio.create_task(sio.emit('tts_state', {'speaking': bool(is_speaking)}))

    # Callback to focus external apps during automation
    def on_focus_mode(active):
        asyncio.create_task(sio.emit('focus_mode', {'active': bool(active)}))
    ...
    try:
        ...
        # Apply current permissions and settings
        audio_loop.update_permissions(SETTINGS["tool_permissions"])
        if "primary_model" in SETTINGS:
            audio_loop._current_brain_model = SETTINGS["primary_model"]
        ...
        def handle_loop_exit(task):
            try:
                ...
                import traceback
                print(f"[SERVER] !!!!! Audio Loop Crashed with exception: {type(e).__name__}: {e} !!!!!")
                traceback.print_exc()
                asyncio.create_task(sio.emit('error', {'msg': f'ADA crashed: {str(e)}'}))
...
async def user_input(sid, data):
    ...
    if text:
        ...
        if not audio_loop.session:
            print("[SERVER DEBUG] Routing text input to Pipeline Mode.")
            asyncio.create_task(audio_loop.process_pipeline_turn(text_prompt=text))
            return
...
async def transcribe_audio_deepgram(audio_bytes, mime_type):
...
@sio.event
async def stt_audio(sid, data):
    audio_b64 = data.get("audio")
    mime_type = data.get("mime_type", "audio/webm")

    if not audio_b64:
        await sio.emit("stt_result", {"error": "No audio received"}, room=sid)
        return

    try:
        audio_bytes = base64.b64decode(audio_b64)
    except Exception as e:
        await sio.emit("stt_result", {"error": f"Invalid audio payload: {e}"}, room=sid)
        return

    async def process_audio():
        try:
            transcript = await transcribe_audio_deepgram(audio_bytes, mime_type)
            if transcript:
                await sio.emit("stt_result", {"text": transcript}, room=sid)
            else:
                await sio.emit("stt_result", {"error": "No speech detected"}, room=sid)
        except Exception as e:
            await sio.emit("stt_result", {"error": str(e)}, room=sid)
...
async def update_settings(sid, data):
    ...
    if "primary_model" in data:
        SETTINGS["primary_model"] = data["primary_model"]
        print(f"[SERVER] Primary model set to: {data['primary_model']}")
        if audio_loop:
            audio_loop._current_brain_model = data["primary_model"]
...
async def get_available_models(sid):
    ...
    print("[SERVER] Fetching available models for client...")
    copilot_models = await ada._get_copilot_models()
    
    # Merge with Groq models
    groq_models = [
        {"id": "llama-3.3-70b-versatile", "name": "Llama 3.3 70B (groq)"},
        {"id": "llama-3.1-8b-instant", "name": "Llama 3.1 8B (groq)"},
        {"id": "llama-3.1-405b-reasoning", "name": "Llama 3.1 405B (groq)"},
    ]
    
    # Ensure primary model is always in the list even if API fails
    all_models = copilot_models + groq_models
    await sio.emit('available_models', {'models': all_models}, to=sid)
