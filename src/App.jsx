...
function App() {
    ...
    const sttRecordingStartRef = useRef(0);
    const sttLastVoiceRef = useRef(0);
    const sttVadFrameRef = useRef(null);
    const sttSendingRef = useRef(false);
    const sttErrorReportedRef = useRef(false);
    ...
    useEffect(() => {
        ...
        if (isConnected && isAuthenticated && socketConnected && !hasAutoConnectedRef.current) {
            ...
            const timer = setTimeout(() => {
                ...
                const deviceName = queryDevice && queryDevice.label ? queryDevice.label : null;
    ...
    useEffect(() => {
        ...
        socket.on('available_models', (data) => {
            if (data && data.models) {
                console.log("[App] Available models received:", data.models.length);
        ...
        socket.on('tts_state', (data) => {
            setTtsSpeaking(Boolean(data?.speaking));
        });
        ...
        socket.on('stt_result', (data) => {
            ...
            if (data?.text) {
                ...
                sendInputText(data.text);
            }
        });

        socket.on('focus_mode', (data) => {
        ...
        return () => {
            ...
            socket.off('tts_state');
            socket.off('focus_mode');
    ...
    const sendSttAudio = async (blob) => {
        if (sttSendingRef.current) return;
        sttSendingRef.current = true;
        try {
            const base64 = await blobToBase64(blob);
            if (!base64) return;
            socket.emit('stt_audio', {
                audio: base64,
                mime_type: blob.type || 'audio/webm'
            });
        } finally {
            sttSendingRef.current = false;
        }
    };
    ...
    const startSttRecording = () => {
        ...
        if (typeof MediaRecorder === 'undefined') {
            if (!sttErrorReportedRef.current) {
                addMessage('System', 'MediaRecorder is not available for speech input.');
                sttErrorReportedRef.current = true;
            }
            return;
        }
        ...
        const recorder = new MediaRecorder(micStreamRef.current, preferredType ? { mimeType: preferredType } : undefined);
        ...
        recorder.onstop = async () => {
            ...
            const durationMs = performance.now() - sttRecordingStartRef.current;
            if (durationMs < 400) return;
            const blob = new Blob(sttChunksRef.current, { type: recorder.mimeType || 'audio/webm' });
            await sendSttAudio(blob);
        };

        sttRecorderRef.current = recorder;
        sttRecordingStartRef.current = performance.now();
        sttLastVoiceRef.current = sttRecordingStartRef.current;
    ...
    const startClientVad = () => {
        if (sttVadFrameRef.current) return;
        if (!analyserRef.current || !micStreamRef.current) {
            return;
        }

        const analyser = analyserRef.current;
        const buffer = new Uint8Array(analyser.fftSize || 64);
        ...
        const tick = () => {
            if (!isConnectedRef.current || isMutedRef.current) {
                stopSttRecording();
                sttVadFrameRef.current = null;
                return;
            }

            if (ttsSpeakingRef.current) {
                stopSttRecording();
                sttVadFrameRef.current = requestAnimationFrame(tick);
                return;
            }
            ...
            if (rms > vadThreshold) {
                sttLastVoiceRef.current = now;
                if (!sttRecordingRef.current) {
                    startSttRecording();
                }
            } else if (sttRecordingRef.current && (now - sttLastVoiceRef.current > silenceMs)) {
                stopSttRecording();
            }
    ...
    const stopClientVad = () => {
        if (sttVadFrameRef.current) {
            cancelAnimationFrame(sttVadFrameRef.current);
            sttVadFrameRef.current = null;
        }
        stopSttRecording();
    };
    ...
    useEffect(() => {
        if (isConnected && !isMuted && !ttsSpeaking) {
            startClientVad();
        } else {
            stopClientVad();
        }
    ...
    }, [isConnected, isMuted, ttsSpeaking, selectedMicId]);
